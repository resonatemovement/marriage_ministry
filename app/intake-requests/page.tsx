...
export default async function IntakeRequestsRoute({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; deleted?: string }> }) {
  ...
  return <WorkspaceShell workspace="admin" activeHref="/intake-requests" identity={identity}><IntakeRequestQueue requests={result.requests} error={result.error} search={params.q} status={params.status} deleted={params.deleted === "1"} /></WorkspaceShell>;
}
