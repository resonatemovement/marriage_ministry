...
import { getSession } from "@/features/session-builder/queries";
...
export default async function SessionDetailPage({ params }: { params: Promise<{ sessionId: string }> }) {
  ...
  if (!session) notFound();
  return <WorkspaceShell workspace={access.workspace} activeHref="/session-builder" identity={access.identity}><main className="mx-auto max-w-[1500px] p-5 sm:p-8 lg:p-10"><p className="font-heading text-xs font-extrabold uppercase tracking-widest text-brand-secondary">Session {session.sequenceNumber}</p><h1 className="font-heading mt-1 text-3xl font-bold text-text-primary">Edit Session</h1><p className="mt-2 text-sm text-text-muted">Manage this session draft and its lifecycle.</p><section className="mt-6 rounded-lg border border-border bg-surface p-5 sm:p-6"><SessionEditor session={session} /></section></main></WorkspaceShell>;
}
