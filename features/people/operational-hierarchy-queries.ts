...
type AssignmentClient = { from: (table: "campus_lead_coach_assignments" | "campus_lead_counselor_assignments") => { select: (columns: string) => { is: (column: string, value: null) => Promise<{ data: Row[] | null; error: { message: string } | null }> } } };

export async function getOperationalHierarchy(): Promise<OperationalHierarchy> {
  const supabase = await createServerSupabaseClient();
  const [teamsResult, leadCoachResult, leadCounselorResult, coachCounselorResult] = await Promise.all([
    ...
    (supabase as unknown as AssignmentClient).from("campus_lead_counselor_assignments").select("campus_lead_group_id,counselor_group_id").is("ended_at", null),
    supabase.from("supervision_assignments").select("coach_group_id,counselor_group_id").is("ended_at", null),
  ]);
  if (teamsResult.error || leadCoachResult.error || leadCounselorResult.error || coachCounselorResult.error) throw new Error("Operational hierarchy is unavailable");
  ...
  return buildOperationalHierarchy(teams, ((leadCoachResult.data ?? []) as Row[]).map((row) => ({ campusLeadId: value(row, "campus_lead_group_id")!, coachId: value(row, "coach_group_id")! })), ((leadCounselorResult.data ?? []) as Row[]).map((row) => ({ campusLeadId: value(row, "campus_lead_group_id")!, counselorId: value(row, "counselor_group_id")! })), ((coachCounselorResult.data ?? []) as Row[]).map((row) => ({ coachId: value(row, "coach_group_id")!, counselorId: value(row, "counselor_group_id")! })));
}
