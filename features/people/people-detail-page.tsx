...
function GroupDetail({ detail, context, trail, isSuperAdmin }: { detail: Extract<PeopleDetail, { kind: "group" }>; context: PeopleDetailActionContext | null; trail: string[]; isSuperAdmin: boolean }) {
  ...
  const counselorPanel = detail.type === "coaches" && context ? <RelationshipCard title="Counselors" rows={context.currentCounselorTeams.map((team) => ({ id: team.id, name: team.name, metadata: `Counselor · ${team.campus ?? "Campus not assigned"}`, href: peopleDetailHref(team.id, trail), action: context.mode === "admin" ? <UnassignOperationalRelationship key={`coach-counselor-${team.id}`} relationship="coach-counselor" sourceId={detail.id} sourceName={detail.name} targetId={team.id} targetName={team.name} /> : undefined }))} empty="No counselors assigned." footer={<PeopleDetailActions detail={detail} context={context} hideEdit compact />} /> : null;
  ...
  const campusLeadCoachesPanel = detail.type === "campus_leads" && context ? <RelationshipCard title="Coaches" rows={context.currentCampusLeadCoaches.map((team) => ({ id: team.id, name: team.name, metadata: `Coach · ${team.campus ?? "Campus not assigned"}`, href: peopleDetailHref(team.id, trail), action: context.mode === "admin" ? <UnassignOperationalRelationship key={`campus-lead-coach-${team.id}`} relationship="campus-lead-coach" sourceId={detail.id} sourceName={detail.name} targetId={team.id} targetName={team.name} /> : undefined }))} empty="No coaches assigned." footer={<PeopleDetailActions detail={detail} context={context} hideEdit compact allowedActions={["assign-campus-lead-coach"]} />} /> : null;
