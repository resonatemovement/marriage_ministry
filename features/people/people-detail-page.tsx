import { Check, Clock3, Mail, Phone, UserRound, UsersRound } from "lucide-react";
...
import { type OperationalStatus, type PeopleDetail, type PeopleDetailMember } from "./detail-model";
...
const peopleDetailMemberAvatarClass = "size-40 rounded-full border-4 border-surface-muted object-cover sm:size-[200px]";
const peopleDetailMemberAvatarSize = 200;
...
function GroupDetail({ detail, context, trail, isSuperAdmin }: { detail: Extract<PeopleDetail, { kind: "group" }>; context: PeopleDetailActionContext | null; trail: string[]; isSuperAdmin: boolean }) {
  ...
  const campusLeadCoachesPanel = detail.type === "campus_leads" && context ? <RelationshipCard title="Coaches" rows={context.currentCampusLeadCoaches.map((team) => ({ id: team.id, name: team.name, metadata: `Coach · ${team.campus ?? "Campus not assigned"}`, href: peopleDetailHref(team.id, trail), action: context.mode === "admin" ? <UnassignOperationalRelationship relationship="campus-lead-coach" sourceId={detail.id} sourceName={detail.name} targetId={team.id} targetName={team.name} /> : undefined }))} empty="No coaches assigned." footer={<PeopleDetailActions detail={detail} context={context} hideEdit compact allowedActions={["assign-campus-lead-coach"]} />} /> : null;
  const campusLeadCounselorsPanel = detail.type === "campus_leads" && context ? <RelationshipCard title="Counselors" rows={context.currentCampusLeadCounselors.map((team) => ({ id: team.id, name: team.name, metadata: `Counselor · ${team.campus ?? "Campus not assigned"}`, href: peopleDetailHref(team.id, trail), action: context.mode === "admin" ? <UnassignOperationalRelationship relationship="campus-lead-counselor" sourceId={detail.id} sourceName={detail.name} targetId={team.id} targetName={team.name} /> : undefined }))} empty="No counselors assigned." footer={<PeopleDetailActions detail={detail} context={context} hideEdit compact allowedActions={["assign-campus-lead-counselor"]} />} /> : null;
