...
export async function getPeopleDetailActionContext(detail: PeopleDetail, mode: "admin" | "campus_lead" = "admin"): Promise<PeopleDetailActionContext> {
  ...
  const [campusesResult, counselorResult, coachResult, currentResult, selfTeamResult, couplesResult, coupleInvitationsResult, campusLeadsResult, coupleReadinessResult, coupleReadinessInvitationsResult] = await Promise.all([
  ...
  if (campusesResult.error || counselorResult.error || coachResult.error || currentResult.error || selfTeamResult.error || couplesResult.error || coupleInvitationsResult.error || campusLeadsResult.error || coupleReadinessResult.error || coupleReadinessInvitationsResult.error) {
  ...
  return {
    ...
    campusLeadCoachTeams: detail.kind === "group" && detail.type === "campus_leads" ? eligibleCampusLeadCoachTeams(coachTeams, detail.campusId) : [],
