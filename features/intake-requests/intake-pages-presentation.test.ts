...
vi.mock("./status-manager", () => ({
  IntakeStatusManager: ({ isSuperAdmin }: { isSuperAdmin: boolean }) => createElement("section", null, "Start Review", "Send Invite", "Close Request", "Reopen for Review", isSuperAdmin ? "Delete" : null),
}));
...
const request: IntakeRequestDetail = {
  id: "request-a", status: "ready_for_review", hasInvitedCouple: false, relationshipStatus: "engaged", campusName: "Campus A", requestedSupport: ["lay_counselor"], submittedAt: "2026-09-10T00:00:00.000Z",
  people: [
...
describe("Intake request presentation capabilities", () => {
  it("gives a Campus Lead normal workflow controls without permanent deletion", () => {
    ...
    expect(page).not.toContain("Delete</section>");
  });
  ...
  it("shows permanent deletion only to an eligible Super Admin", () => {
    ...
    expect(page).toContain("Delete</section>");
  });
});
