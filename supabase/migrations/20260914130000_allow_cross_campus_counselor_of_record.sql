...
begin
  ...
  if target_assignment_type = 'counselor' then
    select group_type into target_group_type from public.groups where id = target_group_id and active;
    if target_group_type not in ('counselor_team', 'coach_team', 'campus_lead_team') then raise exception 'Choose an active Counselor, Coach, or Campus Lead team'; end if;
