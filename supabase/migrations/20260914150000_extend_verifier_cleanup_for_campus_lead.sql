...
declare group_ids uuid[]; case_ids uuid[]; lead_coach_count integer; supervision_count integer; assignment_count integer; case_history_count integer; case_count integer;
begin
  ...
  where name like 'verify-cross-campus-counseling-%'
    ...
    -- The legacy verifier contract has a numeric run id immediately after its
    -- prefix. This deliberately excludes verify-campus-lead-coach-campus-.
    or name ~ '^verify-campus-lead-[0-9]+([[:space:]-]|$)'
    -- Campus Lead invitation groups are named after invitees, rather than the
    -- verifier run. Their exact verifier-owned invitation email proves ownership.
    or id in (
      select group_id
  ...
  return jsonb_build_object('campus_lead_coach_assignments', lead_coach_count, 'supervision_assignments', supervision_count, 'case_assignments', assignment_count, 'case_status_history', case_history_count, 'counseling_cases', case_count);
end;
$$;
