...
begin
  if actor is null or not private.current_user_can_manage_intake(target_request_id) then
    raise exception 'Not authorized to invite this Intake Request';
  end if;
  ...
  if current_request.status = 'invited' and current_request.invited_group_id is not null then
    ...
    where group_id = current_request.invited_group_id;
    return jsonb_build_object('group_id', current_request.invited_group_id, 'invitation_ids', invitation_ids, 'created', false);
  end if;

  if current_request.status <> 'under_review' or current_request.invited_group_id is not null then
    raise exception 'Intake Request is not ready to invite';
  end if;
  if current_request.campus_id is null or not exists (select 1 from public.campuses where id = current_request.campus_id and active) then
    raise exception 'Intake Request requires an active campus before inviting';
  end if;
  ...
  if requester.id is null or partner.id is null or requester.email = partner.email then
    raise exception 'Intake Request must have two people with different emails';
  end if;
  if exists (select 1 from auth.users where lower(email) in (requester.email, partner.email))
    or exists (select 1 from public.profiles where email in (requester.email, partner.email))
    or exists (select 1 from public.invitations where email in (requester.email, partner.email) and status = 'pending') then
    raise exception 'An active user or pending invitation already exists for an Intake Request email';
  end if;

  insert into public.groups (campus_id, group_type, name)
  values (current_request.campus_id, 'couple', 'Pending Couple invitation')
  ...
  for person in select * from public.intake_request_people where intake_request_id = target_request_id order by person_position loop
    ...
    values (person.email, person.first_name, person.last_name, person.phone, 'couple', current_request.campus_id, group_id, actor)
    ...
    values (actor, 'invitation.created', 'invitation', invitation_id, jsonb_build_object('role', 'couple', 'campus_id', current_request.campus_id, 'group_id', group_id));
  end loop;
  ...
  insert into public.audit_events (actor_id, event_type, entity_type, entity_id, details)
  values (actor, 'intake_request.invited', 'intake_request', target_request_id, jsonb_build_object('group_id', group_id, 'invitation_ids', invitation_ids));
