...
begin
  ...
  if matching_invitations > 1 then
    raise exception 'Invitation identity is ambiguous';
  elsif matching_invitations = 1 then
    select * into invitation from public.invitations where auth_user_id = actor for update;
  else
    select count(*) into matching_invitations from public.invitations
    where auth_user_id is null and email = actor_email and status = 'pending';
    if matching_invitations > 1 then
      raise exception 'Invitation identity is ambiguous';
    elsif matching_invitations = 1 then
      select * into invitation from public.invitations
      where auth_user_id is null and email = actor_email and status = 'pending' for update;
    elsif exists (select 1 from public.invitations where email = actor_email and auth_user_id is not null and auth_user_id <> actor) then
  ...
  if not exists (select 1 from public.campuses where id = invitation.campus_id) then raise exception 'Invitation campus is invalid'; end if;
  ...
  if invitation.intended_role in ('couple', 'coach', 'counselor') then
    ...
    if exists (
      ...
      join public.groups existing_group on existing_group.id = membership.group_id
      where membership.profile_id = actor and membership.ended_at is null
        and membership.group_id <> invitation.group_id and existing_group.group_type = group_kind
    ) then raise exception 'Profile already belongs to another active group of this type'; end if;
  ...
  on conflict (id) do update set
    ...
    status = case
      when public.profiles.status = 'active' then 'active'::public.profile_status
  ...
  if invitation.status = 'pending' then
    update public.invitations set status = 'accepted', accepted_at = now(), auth_user_id = actor where id = invitation.id;
    accepted_now := true;
  end if;
...
revoke execute on function public.accept_invitation() from public, anon, authenticated;
